...
async def update_cancion(
    ...
    updated_song = song_service.update_song(cancion_id, update_data=update_data)

    if updated_song is None:
        raise HTTPException(status_code=404, detail=f"Canción con id={cancion_id} no encontrada")
    
    return updated_song


@router.patch(
    "/canciones/{cancion_id}/precio",
    response_model=CancionOut,
...
async def update_cancion_price(
    ...
    song_service = SongService(db)
    updated_song = song_service.update_song_price(cancion_id, precio=price_update.precio)
    if updated_song is None:
        raise HTTPException(status_code=404, detail=f"Canción con id={cancion_id} no encontrada")
    return updated_song
